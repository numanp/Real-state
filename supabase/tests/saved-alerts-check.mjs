...
ok('a fresh search shows 0 new matches', (await myCount(A.c)) === 0);
...
await prop({});
const afterMatch = await myCount(A.c);
ok('a new matching listing bumps the in-app count', afterMatch >= 1, `count=${afterMatch}`);

await prop({ listing_type: 'buy' });
ok('a non-matching listing does not bump the count', (await myCount(A.c)) === afterMatch);

// isolation: another user does not see A's search, and cannot mark it seen.
const B = await signUp('inapp_b');
const { data: bList } = await B.c.rpc('my_saved_search_alerts');
ok("another user's alerts do not include A's search", !(bList ?? []).some((r) => r.saved_search_id === sid));
await B.c.rpc('mark_saved_search_seen', { p_saved_search_id: sid });
ok("a different user cannot reset A's watermark", (await myCount(A.c)) === afterMatch);

// owner marks seen → count resets to 0.
await A.c.rpc('mark_saved_search_seen', { p_saved_search_id: sid });
ok('marking the search seen resets the count to 0', (await myCount(A.c)) === 0);
